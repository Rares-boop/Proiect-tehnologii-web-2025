...
function Home() {
  ...
  const handleRemoveTester = async (projectId, e) => {
    ...
    if (
      !window.confirm(
        "Are you sure you want to stop being a tester for this project?"
      )
    ) {
      return;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4 sm:mb-6">
          {user
            ...
            : "Welcome to Bug Bounty Application"}
        </h2>
        {!user && <p className="text-gray-600 mb-6">Login to get started</p>}
        {user && (
            ...
            {projects.length === 0 ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {projects.map((project) => {
                  ...
                  const isMember =
                    user?.role === "MP" && Number(project.is_member) === 1;
                  ...
                  return (
                    ...
                    >
                      ...
                      {project.repository && (
                        <p className="text-xs sm:text-sm text-blue-600 mb-2 break-all">
                          {project.repository}
                        </p>
                      )}
                      <p className="text-xs sm:text-sm text-gray-500">
                        Created by: {project.creator_email}
                      </p>
