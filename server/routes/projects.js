...
router.post('/', authenticateToken, requireMP, async (req, res) => {
    try {
        ...
        if (team_members && Array.isArray(team_members) && team_members.length > 0) {
            const validMemberIds = team_members.filter(id => id && !isNaN(parseInt(id)) && parseInt(id) !== req.user.id);
            
            for (const memberId of validMemberIds) {
                const user = await db.get('SELECT id, role FROM users WHERE id = ? AND role = ?', [memberId, 'MP']);
                if (user) {
                    try {
                        await db.run(
...
router.get('/:id/members', authenticateToken, async (req, res) => {
    try {
        ...
        if (project.created_by !== req.user.id) {
            return res.status(403).json({ message: 'You can only view members of your own projects' });
        }
...
router.post('/:id/members', authenticateToken, requireMP, async (req, res) => {
    try {
        ...
        if (project.created_by !== req.user.id) {
            return res.status(403).json({ message: 'You can only add members to your own projects' });
        }

        if (parseInt(user_id) === req.user.id) {
            return res.status(400).json({ message: 'You are already the project creator' });
        }

        const user = await db.get('SELECT id, role FROM users WHERE id = ? AND role = ?', [user_id, 'MP']);
        if (!user) {
            return res.status(404).json({ message: 'User not found or is not an MP' });
        }
...
router.delete('/:id/members/:userId', authenticateToken, requireMP, async (req, res) => {
    try {
        const projectId = parseInt(req.params.id);
        ...
        if (project.created_by !== req.user.id) {
            return res.status(403).json({ message: 'You can only remove members from your own projects' });
        }
    ...
    try {
        ...
        if (req.user.role === 'MP') {
            ...
            if (!isCreator && !isMember) {
                return res.status(403).json({ message: 'You can only view bugs for projects you are part of' });
            }
        }
